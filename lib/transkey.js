  ...
  async function typePassword(inputId, password) {
    ...
    function findShiftKey(keyList) {
      return keyList.find(k => k.label && (
        k.label.includes('쉬프트') || k.label.includes('shift') ||
        k.label.includes('대문자') || k.label.includes('caps')
      ));
    }
    ...
    for (const char of password) {
      ...
      if (isUpper) {
        const shiftKey = findShiftKey(keys);
        if (shiftKey) {
          console.error(`    Pressing shift (${shiftKey.label}, index: ${shiftKey.index}) for uppercase '${char}'`);
          if (shiftKey.index >= 0) await clickByIndex(shiftKey.index);
          else await clickXY(shiftKey.x, shiftKey.y);
          await sleep(400);
          const shiftedKeys = (await readLayout()) || [];
          applyLayout(keys, labelMap, shiftedKeys);
        }
      }
