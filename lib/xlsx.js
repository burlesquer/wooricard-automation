...
// Month label written in the B column of each data row (e.g. "6월").
const MONTH_LABEL_REGEX = /^\d{1,2}월$/;
...
function createXlsxStore(filePath, { columns = DEFAULT_COLUMNS } = {}) {
  ...
  async function createMonthSheet(newYearMonth) {
    ...
    try {
      ...
      // Update month label column (default B): cloned rows still carry the previous month's label.
      // Only rewrite cells that look like a month label ("6월") — leave "합계" and other text alone.
      const newMonthLabel = `${month}월`;
      newSheet.eachRow({ includeEmpty: false }, (row) => {
        const bCell = row.getCell(COL.monthLabel);
        const v = bCell.value;
        const text = typeof v === 'string'
          ? v
          : (v && typeof v === 'object' && Array.isArray(v.richText) ? v.richText.map((t) => t.text).join('') : null);
        if (!text || !MONTH_LABEL_REGEX.test(text.trim())) return;
        if (v && typeof v === 'object' && Array.isArray(v.richText)) {
          bCell.value = { richText: [{ font: v.richText[0].font, text: newMonthLabel }] };
        } else {
          bCell.value = newMonthLabel;
        }
      });
